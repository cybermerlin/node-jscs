...
module.exports.prototype = {
	...
	check: function (file, errors) {
		var _this = this,
				...
				indentChar = this._indentChar,
				indentSize = this._indentSize,
				rNotIndentChar = new RegExp('[^ ^\t]'),
		...
		/**
		 * возвращает номер позиции первого в строке символа, не используемого в качестве отступа (не пробел и не таб)
		 * @param line
		 * @return {Number}
		 * @private
		 */
		...
		function setModuleBody(node) {
			...
			if (moduleBody && moduleBody.body.length) {
				_this._moduleIndents = moduleBody.body[0].loc.start.column > 0 ? ++_this._moduleIndents : 0;
			}
		}
		...
		linesToCheck = file.getLines().map(function (line) {
			return {
				push:        [],
				pushAltLine: [],
				pop:         [],
